...
Refit the random walk model from above, but pretend it is 14 days earlier than the end of the outbreak.
You can do this by truncating the observations:

```{r truncate-data, eval = FALSE}
...
Extract the $R_t$ posterior as before and plot it alongside the full-data $R_t$ posterior.
...
The $R_t$ estimates from the truncated fit have much wider uncertainty in the last few days before the cutoff.
...
The truncated fit's median also tends to flatten out near the cutoff: with no data telling it $R_t$ is changing, the random walk just continues from the last well-informed value, so in real time you can get both wider uncertainty *and* a biased point estimate.
:::
