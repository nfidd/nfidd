...
In this session we will use the `nfidd` package to load a data set of infection times and built-in stan models, the `ggplot2` package for plotting, the `dplyr` and `tidyr` packages to wrangle data, the `lubridate` package to deal with dates, and the `cmdstanr` package for using stan.
...
We saved the code we used in that session to simulate symptom onset and hospitalisation data.
...
cmod <- nfidd_cmdstan_model("censored-delay")
cmod
```
