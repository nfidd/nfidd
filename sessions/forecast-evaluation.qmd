...
log_sc_forecasts |>
  ...
  ggplot(aes(x = mid, y = density)) +
  geom_col() +
  labs(title = "PIT histogram, scored on the log scale", x = "Predicted quantiles")
```
...
log_sc_forecasts |>
  ...
  geom_col() +
  facet_wrap(~group_horizon) +
  labs(title = "PIT by forecast horizon, scored on the log scale", x = "Predicted quantiles")
```
...
log_sc_forecasts |>
  ...
  geom_col() +
  facet_wrap(~target_day) +
  labs(title = "PIT by forecast date, scored on the log scale", x = "Predicted quantiles")
```
