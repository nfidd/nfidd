...
According to the help page obtained by running `?accuracy.fbl_ts`: "Accuracy measures can be computed directly from models as the one-step-ahead fitted residuals are available." 
When we computed the "Training" set results, we just passed `accuracy()` the model object, so it is computing the metrics only on 1-step ahead forecasts, which generally will be more accurate since they are "closer" to observed data.
:::
...
3. a model that just has fourier terms.
...
::: {.callout-note collapse="true"}
## Solution
- The CRPS scores for all models increase with forecast horizon.
At horizons 1 and 2 the errors are similar, alhough the `rw` model has moderately higher CRPS. 
...
- By forecast_date, the models show similar forecast error for dates early and late in the season with the larger differences coming at the weeks with higher incidence, especially around the "second peak" of the data. 
