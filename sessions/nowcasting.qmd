...
::: {.callout-important}
## The three dimensions of epidemiological data

So far we've worked with data that has two time dimensions:
- **Infection time**: when transmission occurred
- **Onset time**: when symptoms began

Now we introduce a third dimension:
- **Report time**: when the case enters our surveillance system

This creates an observational challenge: **we observe data by report time, but we want to understand patterns by epidemiologically meaningful time** (onset, hospitalisation, etc.).
:::
