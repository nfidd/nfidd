...
::: {.callout-note collapse="true"}
## What happens when we average forecasts from statistical and mechanistic models? (optional)

In the above we have compared the mechanistic and statistical models. What happens if we use a forecast which is a average of the two models? We can do this by combining the forecasts from the two models and then scoring the combined forecast.

```{r}
combined_forecasts <- bind_rows(
    mutate(mech_forecasts, model = "combined") |>
    mutate(sample = 1:n()),
    mutate(stat_forecasts, model = "combined") |>
    mutate(sample = 1:n())
  ) |>
  group_by(target_day, horizon, sample, model) |>
...
What do you think of the combined forecast?

::: .callout-note collapse="true"
## Solution
:::

We could also make a truely combined joint model which uses the mechanistic model to inform the statistical model and vice versa. This is a more complex model but would likely give better forecasts.
:::
