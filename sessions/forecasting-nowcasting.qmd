...
ggplot() +
  geom_line(data = complete_at_horizon, 
            aes(x = day, y = complete_onsets, colour = "As of two weeks time"), 
            linewidth = 1) +
...
We start by filtering to "complete" data only (more than 14 days old):

```{r complete-data-filter}
complete_data_forecast_data <- naive_data
head(complete_data_forecast_data)
```

Next, we load the renewal forecasting model:
...
- The model is the same as the one we used in the [Rt estimation session](sessions/R-estimation-and-the-renewal-equation), but with an additional block where we generate the forecast for additional days beyond the observation period. To do this we need to regenerate the Rt samples for the additional days, and then use these to generate infections and onsets.
...
complete_data_fit <- nfidd_sample(
  complete_data_model,
  data = list(
    n = nrow(complete_data_forecast_data),
    I0 = 10,  # Initial number infected
    obs = complete_data_forecast_data$available_onsets,
...
complete_data_forecasts <- complete_data_fit |>
  ...
  mutate(
    actual_day = day + max(complete_data_forecast_data$day),
...
We start by fitting the geometric random walk nowcast to the reporting triangle data, as we did in the [joint nowcasting session](joint-nowcasting).
...
geometric_nowcast_fit <- nfidd_sample(
  ...
  data = list(
    ...
    ip_max = length(ip_pmf) - 1,
    ip_pmf = ip_pmf,
    h = 0  # No forecasting in this step, just nowcasting
  )
)
```
...
pipeline_data <- bind_rows(
  ...
  nowcast_estimates |> filter(day > max(naive_data$day)) |> select(day, onsets = nowcast)
) |>
  arrange(day)
...
First, we load the renewal forecasting model:

```{r pipeline-forecast-model}
...
pipeline_forecast_fit <- nfidd_sample(
  pipeline_forecast_model,
  data = list(
    n = nrow(pipeline_data),
    I0 = 10,  # Initial number infected
    obs = round(pipeline_data$onsets),
    gen_time_max = length(gen_time_pmf),
    gen_time_pmf = gen_time_pmf,
    ip_max = length(ip_pmf) - 1,
    ip_pmf = ip_pmf,
...
We start by loading the joint model (note: this assumes the model supports forecasting parameters):
...
ggplot(joint_forecasts, aes(x = day)) +
  geom_col(
