...
```{r discretised_gamma}
gamma_pmf <- discretise(pgamma, 14, shape = 5, rate = 1)
gamma_pmf
```
:::
...
onsets <- convolve_with_delay(inf_time_series$n, gamma_pmf)
```
:::

We can plot these symptom onsets
```{r convolution_plot}
combined <- inf_time_series |>
  rename(date = infection_date, infections = n) |>
  mutate(onsets = onsets)
ggplot(combined, aes(x = date, y = onsets)) +
