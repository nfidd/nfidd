...
We will try to make a forecast on day 71 (assuming we do not know what the data looks like after that day) as in the nowcasting session.
...
::: {.callout-note collapse="true"}
## Solution
- Interpretable reference point: yes, it is just a naive extrapolation of the recent trend in $R_t$.
- Cheap to fit and robust: not really. It is a full Bayesian latent-state model that takes appreciably longer to fit than a naive baseline, and we had to raise `adapt_delta` and supply initial values to sample it cleanly.
