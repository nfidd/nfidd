...
The project collected nearly 1 billion rows of model output data, and has led to multiple research papers on epidemiological forecasting.[@ray_comparing_2023;@cramer_evaluation_2022;@fox_optimizing_2024]
...
In this session, we will access and analyse forecasts from the CFA-run COVID-19 Forecast Hub.
...
Ideally, storing data in the hubverse ensures that the data is "clean", no typos in targets or locations, no missing quantile levels, no negative predictions, etc...
...
-   we have forecasts for horizons -1 to 3 weeks, 
...
The `covid_locations` data object stored in this package was downloaded and stored from the metadata of the COVID-19 Forecast Hub.
The following code merges state abbreviations into both the forecasts and time-series datasets.
...
This is a key part of [the forecasting paradigm](forecast-evaluation.html#the-forecasting-paradigm) mentioned in the Forecast Evaluation session is that we want to maximize sharpness subject to calibration.
...
This is the inject a bit of realism: how could we know in real-time how our forecast did for the current week?
...
- assign equal weights for the first two weeks. 
(There are missing forecasts here, and we haven't learned much about the model performance yet anyways.)
...
first_week_idx <- weights_per_model$target_end_date == as.Date("2024-11-16")
weights_per_model[first_week_idx, "weight"] <- 1/sum(first_week_idx)

second_week_idx <- weights_per_model$target_end_date == as.Date("2024-11-23")
...
As always, it is good to plot your data as a sanity check. Here are the model weights plotted over time.
...
::: callout-tip
### Take 5 minutes
Looking at the above plot, what is the "story" that the weights over time tell? 
...
But this omits some interesting models, including the model from Metaculus, a human-judgment forecast aggregation company, and some models from the CDC Center for Forecasting and Outbreak Analytics (CFA).
