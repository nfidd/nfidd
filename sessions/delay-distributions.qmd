...
- **Impact of sample size**: Try subsampling your data to see how fewer observations affect your parameter estimates and uncertainty. For example:
  ```{r, eval = FALSE}
  # Subsample to 100 observations
  df_small <- df_onset_to_hosp |> slice_sample(n = 100)
  # Refit the model and compare results
  ```

- **Different delay lengths**: The updated `add_delays()` function now allows you to change the delay parameters. Try:
  ```{r, eval = FALSE}
  # Longer delays
  df_long <- add_delays(infection_times, 
                       hosp_params = list(meanlog = 2.5, sdlog = 0.5))
  # How does this affect your parameter recovery?
  ```

- **Different variability of the delay**: Try changing the variability of the delay:
  ```{r, eval = FALSE}
...
- **Different type of distribution**: Try using a completely different distribution:
  ```{r, eval = FALSE}
