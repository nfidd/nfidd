...
The gamma distribution is a *continuous* distribution, but now our delay data are in days which are *discrete* entities. In addition to this, we are assuming that both events that make up the delay (here infections and onsets) are observed as daily counts.
This means that our distribution is **double interval censored** which we encountered in the [the biases in delay distribution session](sessions/biases-in-delay-distributions.qmd) and so we can need to use the same ideas introduced in that session. The important point to remember is that the cumulative distribution functon ($F(t)$) of a distribution that has a daily censored primary event can be expressed as,
$$
F^*(t) = \int_0^1 F(t - u) du
$$

The probability mass function (PMF) is then

$$
f_t \propto \frac{F^*(t + 1) - F^*(t - 1)}{}
$$

which you may be familiar with from single censored (i.e. for the secondary event) discretised distributions.

We can also think about this via simulation.
...
This means that discretising a delay in this context is **double censoring** as we discussed i
