...
In this session, we will start with the renewal model that we have been using and explore adding both more mechanistic structure and then more statistical structure to the model.
We will again evaluate these models to see what effect these different approaches might have.
...
This is the idea behind the _SIR model_ which is a simple compartmental model of infectious disease transmission.
...
We have coded this up as a stan model in `stan/mechanistic-r.stan`.
...
Adding more mechanistic structure is not always possible and, if we do not specify mechanisms correctly, might make forecasts worse.
...
This becomes a random walk when the multiplied value is 0.
...
However, we probably do not want a model which is biased towards a reproduction number of 1 (unless we have good reason to believe that is the expected behaviour).
So what should we do?

Returning to the idea that the reproduction number is a _non-stationary_ time series, as we expect to have a trend in the reproduction numbers we want to capture, we can use a method from the field of time series analysis called _differencing_ to make the time series stationary.
...
We have coded up a model that uses this differenced AR process as a stan model in `stan/statistical-r.stan`.
...
We will now use the mechanistic and statistical models to forecast the number of cases in the future using data simulated in the same way as we did in the [forecasting concepts](forecasting-concepts) session.
...
::: {.callout-note collapse="true"}
## Solution
- The more mechanistic model captures the trend in the data very well.
...
As in the [forecast evaluation session](forecast-evaluation), we have fit these models to a range of forecast dates so you do not have to wait for the models to fit.
...
- If we were not confident in the effective susceptible population the AR model might be preferable.
:::
...
We can now use the `scoringutils` package to calculate some metrics as we did in the [forecasting evaluation session](forecast-evaluation).
...
Let us summarise the scores by model first.
...
Let us first look at the overall PIT histogram.
...
As before let us look at the PIT histogram by forecast horizon (to save space we will group horizons)
...
## What happens when you are very wrong about the population size?

In the forecasts above we assumed that we knew the population size roughly.
...
- The statistical models we have discussed here represent just a subset of available forecasting approaches.
For a comprehensive treatment of time series forecasting models, including ARIMA models as a generalisation of autoregressive approaches with additional features for differencing and moving averages, see @hyndmanForecastingPrinciplesPractice2021. 
