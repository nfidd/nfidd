...
```{=html}
<iframe class="nfidd-embed" src="../interactive/nowcasting.html?embed=build,triangle" title="nowcasting interactive" loading="lazy" style="width:100%;height:1200px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
...
See each day's cases split into observed + missing and summed into a nowcast, plus the reporting triangle. [Open the full explainer ↗](../interactive/nowcasting.html){target="_blank"}
:::

## The simplest possible nowcasting model

::: {.callout-tip .optional collapse="true"}
## Interactive: the simplest nowcast (divide by the CDF)

```{=html}
<iframe class="nfidd-embed" src="../interactive/nowcasting.html?embed=simple" title="the simplest nowcast (divide by the CDF) interactive" loading="lazy" style="width:100%;height:700px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
<script>(function(){if(window.__nfiddEmbedInit)return;window.__nfiddEmbedInit=1;window.addEventListener('message',function(ev){var h=ev&&ev.data&&ev.data.nfiddEmbedHeight;if(!h)return;document.querySelectorAll('iframe.nfidd-embed').forEach(function(f){if(f.contentWindow===ev.source)f.style.height=(h+24)+'px';});});})();</script>
```

Take the observed counts and divide by the proportion reported F(T − t) to recover the eventual total — the band flares where few reports are in. [Open the full explainer ↗](../interactive/nowcasting.html){target="_blank"}
:::
