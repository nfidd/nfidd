...
We will now use the mechanistic and statistical models to forecast the number of cases in the future using data simulated in the same way as we did in the [forecast visualisation session](forecast-visualisation). We will first load in the data and filter for a target forecast date.
...
Everything seems to be in order. We can now use the `scoringutils` package to calculate some metrics.
