...
In this session, we will start with forecasts from the models we explored in the [forecasting models](forecasting-models) session and build ensembles of these models. 
...
As in the session on [forecast evaluation](forecast-evaluation), we have fitted these models to a range of forecast dates so you do not have to wait for the models to fit.
...
::: {.callout-note}
The **weighted interval score (WIS)** [@bracherEvaluatingEpidemicForecasts2021] is a proper scoring rule for quantile forecasts that approximates the **Continuous Ranked Probability Score (CRPS)** by considering a weighted sum of multiple prediction intervals.
...
- As the random walk model is performing poorly, you would expect it to have low weights but actually it often does not.
...
- Throughout the course we have been carefully building models from our understanding of the underlying data generating process.
