...
However, this approach had problems: we did not fully account for uncertainty, or for example observation error in the primary events, and it is not a fully generative model of the data reporting process. 
...
In this session, we will introduce this approach to joint estimation in nowcasting. 
At the end we will then demonstrate a way to combine this with our previous work estimating the reproduction number, steadily improving our real time outbreak model.
...
Now we will simulate the same process but at the population level with observation error - for the same reasons we made this change in the [convolutions session](sessions/using-delay-distributions-to-model-the-data-generating-process-of-an-epidemic).

First, let us generate our simulated onset dataset as before:
...
To recover the onsets by day we group by day and then sum reported onsets across report days.
...
As we only observed reported onsets up to the current day we need to filter it to only include the data we have observed:
...
Finally, we sum this to get the counts we actually observe.
This is the same as the date we corrected for **right truncation** in the [nowcasting concepts session](sessions/nowcasting).
...
So far we have worked with data that has two dimensions.
Such as:

- **Onset day**: the day that each infected case saw symptoms begin
- **Onset counts**: the number of cases with symptoms that began on each day

Now we are introducing a third dimension:

- **Report day**: when the symptomatic case enters our surveillance reporting system

This means that to recover the true onset counts we need to sum across the third dimension, but we may not be able to do this if all the data has not been reported yet (i.e. we have right truncation in our data).
...
A key insight is that, at the current day $T$, the number of reports $n_{t, d}$ for each reference day $t$ at delay $d$ splits into those we have **already observed** (short delays, $d \leq T-t$) and those still **missing** (long delays, $d > T-t$).
Summing over delay, the eventual total $N_t$ therefore splits into an observed and a missing part:

$$
N_{t} = \underbrace{\sum_{d=0}^{T-t} n_{t,d}}_{N_t^{\text{obs}}\ \text{(reports already in)}} \; + \; \underbrace{\sum_{d=T-t+1}^{D} n_{t,d}}_{N_t^{\text{miss}}\ \text{(to be estimated)}}
$$

where:

- $N_t^{\text{obs}}$ is what we have observed by the current day $T$ (the cells with $t+d \leq T$)
- $N_t^{\text{miss}}$ is what we still need to estimate (the cells with $t+d > T$)
...
This data structure is sometimes called the "reporting triangle" in the literature because when visualised as a matrix (with onset days as rows and reporting days as columns), the observed cells (those with $t + d \leq T$) create a triangular shape.

![Reporting triangle visualisation (by Johannes Bracher)](slides/figures/reporting_triangle.png)

Nowcasting aims to complete this triangle by estimating the missing cells (those with $t + d > T$), and so the missing sum $N_t^{\text{miss}}$ that completes each row's total $N_t$.
...
## Model details
This time we will not go into details of the model.
...
Another new feature is the `generated quantities` block.
This block runs once per posterior draw to compute quantities derived from the fitted parameters, rather than contributing to the likelihood.
Here we use it to produce the nowcast: we reconstruct the complete reporting triangle (onsets by all delays, including those not yet observed) and combine the observed counts with model predictions for the not-yet-reported parts to estimate the true onsets.
:::
...
joint_data <- list(
  ...
  d = 16               # max number of days that we know have some reporting delay
)
...
- When reporting delays do not provide enough information to identify recent trends, using the underlying generative model (renewal process) prevents bias
...
They avoid issues of passing fixed estimates between the nowcast and the reproduction number estimation model.
:::
...
We recommend reading through it to get a sense of how all the pieces fit together.
:::
...
As in the [renewal session](sessions/R-estimation-and-the-renewal-equation), we need to define the generation time distribution and an incubation period distribution.
We will use the same distributions as in the [renewal session](sessions/R-estimation-and-the-renewal-equation) for simplicity.
These are:
...
::: {.callout-note collapse="true"}
## Solution
Essentially rather than using `observe_onsets_with_delay.stan` we would use `condition_onsets_by_report.stan` and pass in the proportion reported as data.
This would allow us to use the simple nowcasting model whilst still estimating the reproduction number.
We would also remove the `generated quantities` block as we no longer reconstruct unobserved onsets via the reporting-delay model; we are not nowcasting the data, so simplify the observations to just the number of onsets.
:::

Now let us fit the final model for this session!
...
- The key takeaway from the reproduction number plot is that it looks similar to the one we estimated in the [renewal session](sessions/R-estimation-and-the-renewal-equation).
