...
In this session we will use the `nfidd` package to load the data set of forecasts, the `dplyr` and `tidyr` packages for data wrangling, and `ggplot2` library for plotting. We also use `qra` for quantile regression averaging in the weighted ensemble section.
...
- It does this by using a set of default quantiles but these can be overridden.
...
The key tradeoff to consider is how much to optimise for which models have performed well in the past (and what your definition of the past is, is it all time, or only the last few weeks) versus how much you want to allow for the possibility that these models may not perform well in the future.
...
In the real world you would not do this as you would not have access to the test data and so this is a "best case" scenario.
:::
...
Lets score the filtered ensembles.
...
There are many ways to weight the forecasts in a quantile ensemble. Here we will explore two common methods: 

- Inverse WIS weighting
- Quantile regression averaging

Inverse WIS weighting is a simple method that weights the forecasts by the inverse of their WIS over some period (note that identifying this period is not straightforward).
It's main benefit is that it is simple to understand and implement.
However, it does not optimise the weights directly.

Quantile regression averaging on the other hand does optimise the weights directly and so can be expected to perform better.
...
We need to perform quantile regression averaging (QRA) for each forecast date. We also need to consider how many previous forecasts we wish to use to inform each ensemble forecast. Here we decide to use up to 4 weeks of previous forecasts to inform each QRA ensemble.
...
qra_ensembles_obj <- forecast_dates[-1] |>
  lapply(\(x) quantile_forecasts |>
    mutate(target_end_date = x) |>
    dplyr::filter(target_day <= x) |>
    dplyr::filter(target_day >= x - (4 * 7 + 1)) |>
...
qra_weights <- qra_ensembles_obj |>
  lapply(\(x) attr(x, "weights"))
...
Again lets start by plotting a single forecast from each model and ensemble.
...
Which do you think is best?
Are you surprised by the results? Can you think of any reasons why this might be the case?
:::
...
For a final evaluation we can look at the scores for each model and ensemble again. We remove the first week of forecasts as these do not have a quantile regression average forecasts as these require training data to estimate.

```{r score-weighted-ensembles}
weighted_ensemble_scores <- weighted_ensembles |>
  filter(target_day > 22) |>
  as_forecast_quantile(forecast_unit = c("target_day", "horizon", "model")) |>
  score()
```
...
Remembering back to the [forecasting concepts session](forecasting-concepts), we should also check performance on the log scale. 

```{r score-overview-log}
