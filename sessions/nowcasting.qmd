...
Write $n_{t,d}$ for the number of events with reference date $t$ (the day they occurred) that are reported after a delay of $d$ days, and let $p_d$ be the reporting-delay distribution, so that $n_{t,d} \approx N_t\, p_d$ where $N_t$ is the eventual total for date $t$. As of the current date $T$, a report for date $t$ has had time to arrive only if its delay satisfies $d \le T - t$. The count for date $t$ therefore splits into a part we have already observed and a part still to come:
...
We only ever observe $C^{\mathrm{obs}}_{t}$; nowcasting estimates the missing sum. Writing the delay cumulative distribution function as $F(T-t) = \sum_{d=0}^{T-t} p_d$ (the proportion of date $t$'s reports expected in by now), the simplest scaling nowcast is

$$
\widehat{N}_t = C^{\mathrm{obs}}_{t} + \widehat{C}^{\mathrm{miss}}_{t} = \frac{C^{\mathrm{obs}}_{t}}{F(T-t)} .
$$

This is exactly the "divide the most recent counts by the proportion reported" step used below, with the proportion reported being $F(T-t)$.
:::
