...
-   In this session we used `stan` to model a simple delay distribution. We could also consider a range of tools to achieve this: for example, using a regression approach implemented in e.g. [brms](https://cran.r-project.org/web/packages/brms/index.html), or using an R package designed for fitting simple distributions, such as [`fitdistrplus`](https://cran.r-project.org/web/packages/fitdistrplus/index.html) or the 'naive' model in the [`epidist`](https://github.com/epinowcast/epidist). 
-   In this course, we start with building custom models from scratch to help think through the complex epidemiological processes and uncertainties that underlie typical surveillance data. We'll continue to use `stan` to build our understanding of these data-generating processes and explore how these might fit together in more complex models. In practice, there can be a tension between the type of customised process-driven model building we are doing here, and readily available software aiming to account for such processes more generally. Throughout the course, we'll suggest some examples of these tools and methods for you to consider.
