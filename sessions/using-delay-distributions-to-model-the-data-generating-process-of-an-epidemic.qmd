...
This means that discretising a delay in this context is **double censoring** as we discussed in [the biases in delay distribution session](sessions/biases-in-delay-distributions.qmd) and so we can need to use the same ideas introduced in that session.
