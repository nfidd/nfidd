...
In this session we will use the `nfidd` library to load a data set of infection times, the `ggplot2` library for plotting, the `dplyr` and `tidyr` packages to wrangle data, the `here` library to find the stan models, and the `cmdstanr` library for using stan.
...
df <- infection_times |>
  mutate(
    ...
    infection_date = floor_date(start_date + infection_time),
    onset_date = floor_date(start_date + infection_time + incubation_period),
