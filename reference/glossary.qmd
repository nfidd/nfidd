...
-   **Censoring**: When we don't know the exact timing of events. 
    Epidemiological data can be *right censored* (the event is known to have occurred after a certain time), *left censored* (the event is known to have occurred before a certain time), or *interval censored* (the event is known to have occurred within a certain time interval). 
    For example, this might be when an event that happens in continuous time (e.g. infection or symptom onset) is rounded to an integer unit (e.g. calendar date, week) in reported data.
    In epidemiological delay data, censoring can affect either primary or secondary events (*single interval censoring*) or both (*double interval censoring*).
    This can introduce significant uncertainty when we are estimating the time period between two events.

-   **Truncation**: When data are incomplete because of a measurement cut-off.
    Real-time epidemiological data are often right truncated when we use data that has been collected before enough time has passed for all the relevant epidemiological events to occur or be observed. 
    For example, we may be unable to observe intervals (e.g., incubation periods) greater than a threshold (e.g., greater than the number of days elapsed since infection). 
    ...
    This is particularly pronounced during exponentially growing outbreak, making our estimate of the delay distribution artificially short.
