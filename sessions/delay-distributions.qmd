...
This represents a typical situation in the real world, where we may have a model of how an infection has spread, but we don't necessary know how well this corresponds to what really happened.
:::
...
ggplot(infection_times, aes(x = infection_time)) +
  ...
  scale_x_continuous(n.breaks = 45, expand = c(0, 0)) +
...
-   Symptoms to hospital admission:
    ...
    -   *Time from symptom onset to hospital admission*: We assume that the onset-to-hospitalisation period is **lognormally** distributed, with **meanlog 1.75** and **sdlog 0.5**, corresponding to a mean delay of about a week (7 days).
...
- Tip 2: As in the last session, we can use our dataset to sample from the model posterior.

Here we also need to exclude infections that didn't result in hospitalisation (try passing the data to `na.omit()`).
:::
...
# Get data complete data
df_complete <- na.omit(df_onset_to_hosp)
## Use the data to sample from the model posterior
res <- mod$sample(
  data = list(
    n = nrow(df_complete),
    y = df_complete$onset_to_hosp
  ),
