...
So what does this look like in code? Let's load in the model again and take a look.
...
```{r extract-forecast}
# extract "forecast" variable from model output
...
As our forecasting model is based on the effective reproduction number we can also visualise the forecast of the reproduction number. This can be helpful for understanding why our forecasts of onsets look the way they do and for understanding the uncertainty in the forecasts. To get an even better picture here we will fit another model to the longer time series that we have just forecast for, and then compare the forecast and estimated reproduction numbers.
...
forecast_r |>
  ...
  labs(title = "Estimated R", 
       subtitle = "Estimated over whole time series (red), and forecast at day 41 (blue)")
```
...
::: {.callout-note}
The vertical dashed line represents the point at which we started forecasting (blue trajectories).
:::
...
- We used a 14 day forecast horizon starting at each target day.
...
- We used pregenerated simulated data, which is the same as the outbreak we've been using through the course:
...
rw_forecasts |>
  ...
  labs(title = "Symptom onset forecasts, created weekly over an outbreak",
...
Everything seems to be in order. We can now use the `scoringutils` package to calculate some metrics. We will use the default metrics for now and score our forecasts.
...
Whilst the CRPS is a very useful metric it can be difficult to interpret in isolation. It is often useful to compare the CRPS of different models, or to compare the CRPS of the same model under different conditions. For example, let's compare the CRPS across different forecast horizons:
...
and across different forecast start dates:
...
- If it is a U shape then the model is overconfident and if it is an inverted U shape then the model is underconfident. 
...
As before let's look at the PIT histogram by forecast horizon (to save space we will only look at 1, 3, 7, and 14 days)
...
Let's first look at the overall PIT histogram for the log-transformed scores.
