...
Watch the posterior form as a compromise between prior and data — and the data take over as they accumulate. [Open the full explainer ↗](../interactive/bayesian-updating.html){target="_blank"}
:::
...
::: {.callout-tip collapse="true"}
## Interactive: the forecast fan

```{=html}
<iframe class="nfidd-embed" src="../interactive/forecasting-concepts.html?embed=fan" title="the forecast fan interactive" loading="lazy" style="width:100%;height:750px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
<script>(function(){if(window.__nfiddEmbedInit)return;window.__nfiddEmbedInit=1;window.addEventListener('message',function(ev){var h=ev&&ev.data&&ev.data.nfiddEmbedHeight;if(!h)return;document.querySelectorAll('iframe.nfidd-embed').forEach(function(f){if(f.contentWindow===ev.source)f.style.height=(h+24)+'px';});});})();</script>
```

Extend a model forward and watch the uncertainty fan grow with the horizon. [Open the full explainer ↗](../interactive/forecasting-concepts.html){target="_blank"}
:::
