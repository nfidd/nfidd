...
The code below will work more easily if the sandbox hub is downloaded and lives in the same directory as the repository for this course.
...
![A map showing the 10 HHS Regions.](sessions/slides/figures/hhs-regions.png)
...
```{r hub-config}
hub_path <- "../sismid-ili-forecasting-sandbox/"
...
The above table has `r nrow(cv_forecasts)`, which makes sense because there are
