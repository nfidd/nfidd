...
::: callout
Let us look at infectious disease surveillance data from the perspective of an individual infection.
...
We will combine this with the process of infectious disease transmission, with the reproduction number as a key component.
...
-   Accounting for underlying processes can get surprisingly complicated quickly ^[Time travel is [messy stuff](https://youtu.be/q2nNzNo_Xps)], and it is easy to make mistakes
-   There is currently (at the time of devising this course) no comprehensive training resource that links these common questions and challenges
...
-   If you have not already, start with [getting set up for the course](../getting-started)

-   Once you are all set up, we begin with [delay distributions](delay-distributions).

-   For the probability distributions and Bayesian inference with stan that we rely on throughout the course, work through the [Stan reference guide](../reference/stan) as self-study (also shared on Slack); it is not taught as a separate session.
