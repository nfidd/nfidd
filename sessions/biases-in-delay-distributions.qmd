...
Similarly, "admitted on 22 June" could mean any time during that 24-hour period.
...
Let's visualise the difference between these types of censoring using a simple example (*you can change the parameters to see how the bias changes*).
...
# Secondary censoring only (common mistake) - just discretize
secondary_only <- floor(true_delays)

# Double censoring - discretize the primary censored delays
double_censored <- floor(primary_censored)

# Filter to reasonable range and create PMF for discrete data
...
# Set parameters for truncation demo
set.seed(890)
n <- 5000
meanlog <- 1.75
sdlog <- 0.5
...
Once again, we can simulate this effect on our outbreak data, for example by imagining we would like to make an estimate on day 70 of our outbreak.
