...
We now express this relationship via the renewal equation, which links these recent infections to the number of new infections expected on any day via the reproduction number $R$.
...
In most mechanistic models of infectious diseases (starting with the simplest SIR model), $R$ arises out of a combination of parameters and variables representing the system state, for example in a simple SIR model it can be calculated as $R_0 S/N$ where $S$ is the current number of susceptibles in the population of size $N$.
...
ggplot(mapping = aes(x = infection_day)) +
  ...
  geom_line(
    data = rw_inf_posterior, mapping = aes(y = .value), alpha = 0.1
  )
```
...
ggplot(
  all_posteriors,
  mapping = aes(x = infection_day, y = .value, colour = data)
) +
  geom_line(alpha = 0.1) +
