...
So far, we have looked at the uncertainty of the time delays between epidemiological events.
The next challenge is that our information on these delays is usually biased, especially when we are analysing data in real time.
We will consider two types of biases that commonly occur in reported infectious disease data: 
...
In this session, we will introduce censoring and right truncation as typical properties of the process that generates infectious disease data sets, using the delay from symptom onset to hospitalisation as an example.
...
::: callout-note
For this session, we will use **shorter delays** to better demonstrate censoring effects:
...
We will use the `add_delays` function with shorter parameters for the censoring demonstration:
...
So far, the data we have been using has represented time as a *continuous* variable: a numeric time since a given starting point.
...
This makes handling the data easier - we do not have to make any conversions before using the data in stan.
:::
...
It is important to understand that **double interval censoring** can be decomposed into two distinct components:

**Primary event censoring**: Uncertainty about when the first event (symptom onset) occurred within its reported day. 
Even if we know someone developed symptoms "on 20 June", we do not know if this was at 00:01 or 23:59.
...
We will be spending a lot of time thinking about a potentially "short" 24 hour time interval.
In this session, we will start to get a sense for how variation, uncertainty, and bias within this interval might combine to impact daily outbreak estimates.
...
Let us estimate the time from symptom onset to hospitalisation with the censored data.
...
To help us think about this, let us summarise the original time-based incubation period
...
To properly account for double interval censoring, we need to modify the model to include the fact that we do not know when exactly on any given day the event happened.
...
Let us visualise the difference between double and secondary censoring using a simple example (*you can change the parameters to see how the bias changes*).
...
::: callout-note
For the truncation demonstration, we will switch back to longer delays:
...
Some infected cases have not yet presented with symptoms. 
...
Let us visualise the effect of truncation with a simple example:
...
This figure shows how truncation systematically underestimates the mean delay (dashed lines) by excluding longer delays that have not been observed yet.
...
Let us also check the mean and standard deviation of the truncation-adjusted delay distribution:
...
::: {.callout-note collapse="true"}
## The role of the epidemic growth rate
...
When faced with real epidemiological data, it can be challenging to decide which bias corrections to apply.
The flowchart below provides practical guidance for making these decisions [@charnigaBestPracticesEstimating2024]:
