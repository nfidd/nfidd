...
We will try to make a forecast on day 41 (assuming we don't know what the data looks like after that day) as in the nowcasting session.
...
As our forecasting model is based on the effective reproduction number we can also visualise the forecast of the reproduction number.
...
In order to be able to compare this to the "true" reproduction number estimated once all relevant data is available we will fit the model again but with a later cutoff and then compare the reproduction numbers produced as *forecasts* at the earlier time and *estimates* at the later time.
