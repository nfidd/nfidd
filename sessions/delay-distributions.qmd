...
First we will look at this working forwards from infections in an outbreak.
We will use `R` to probabilistically simulate delays from infection to reporting symptoms and hospitalisations.
Then we will work only from this set of outcome data and use a stan model to estimate the parameters of one specific delay (in this example, symptom onset to hospitalisation).
...
This represents a typical situation in the real world, where we may have a model of how an infection has spread, but we do not necessarily know how well this corresponds to what really happened.
:::
...
1.  We do not normally observe infections directly, but their *outcomes* as symptomatic cases, hospitalisations or other events.
...
For now we do not want to spend too much time manipulating dates in R, but we will get back to working with more realistic surveillance data later.
:::
...
-   Infection to symptoms:
    -   We will assume all infections cause symptoms.
...
-   Symptoms to hospital admission:
    -   We will assume that 30% of symptomatic cases become hospitalised.
...
Let us put these assumptions into practice by simulating some data, adding onset and hospitalisation times (in decimal number of days after the first infection) to the infection times.
We will use random values for each infection from the probability distributions we have assumed above.

::: {.callout-tip}
## Take a few minutes
Throughout the course, we repeatedly use some small chunks of code.
Instead of copying these between sessions, we have put them into functions in the `nfidd` R package that we can use when needed.
...
In reality, we do not have infection dates, and we also have to deal with missing data, incomplete observations, data entry errors etc.
For now, let us just assume we have a data set only including symptom onset times and some hospitalisation times. 

Let us look at a specific problem: we would like to estimate how long it takes for people to become hospitalised after becoming symptomatic.
...
We are using Stan to estimate the parameters of a lognormal distribution (meanlog and sdlog) from our observed delay data. 
...
Let us estimate the onset-to-hospitalisation delay using the simulated data set we created above.
...
::: {.callout-tip}
## Take a few minutes
Look at the summary output above.
What do the columns mean?
What are the values of `meanlog` and `sdlog`?
...
That is a lot of data to plot, so instead, when we produce plots we will use a random sample of 100 iterations.
...
What does this tell us about our uncertainty in the delay distribution?
:::
...
- **Uncertainty visualisation**: The spread of lines shows our uncertainty about the true delay distribution - we are not certain about the exact shape
...
- **Most likely region**: Where lines cluster together shows where we are most confident about the distribution shape
...
With real data, of course, we do not have this information available. 
