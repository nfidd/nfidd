...
censored_delay_pmf <- function(rgen, n = 1e+6, ...) {
  ...
  counts <- table(factor(delay, levels = seq(0, max_delay)))
...
```{r discretised_gamma}
gamma_pmf <- censored_delay_pmf(rgamma, shape = 5, rate = 1)
gamma_pmf
```
:::
...
inf_posterior <- inf_fit |>
  ...
  mutate(infection_date = inf_ts$infection_date)


ggplot(mapping = aes(x = infection_date)) +
  geom_line(data = inf_ts, mapping = aes(y = n)) +
  geom_ribbon(
    data = inf_posterior,
