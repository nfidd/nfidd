...
r_inf_fit <- nfidd_sample(r_inf_mod, data = data)
```
...
Otherwise, the same absolute change in the reproduction number would have a larger effect when the reproduction number is small, which likely does not match your intuition for how outbreaks evolve over time.
...
The infection estimates are the least uncertain from any model and potentially overly certain as they do not fully cover the observed infections.
:::
...
The model that fits directly to infections has the lowest uncertainty, which we would expect as it does not have to infer the number of infections from symptom onsets but even here the reproduction number estimates are unrealistically noisy due to the assumption of independence between infections each day when infection counts are low.
...
# Estimating $R_t$ in real time {.optional}

So far we have fitted the model to the complete outbreak.
In practice, $R_t$ estimation usually happens *during* an outbreak, when the dataset only runs up to the day of estimation.
Estimates near "today" are then poorly informed in two ways: the random walk for $R_t$ has very few downstream observations constraining its latest steps, and the convolution with the incubation period means the symptom onsets caused by very recent infections have not been observed yet.

::: callout-tip
## Try it yourself

Refit the random walk model from above, but pretend it is 14 days earlier than the end of the outbreak by truncating the observations (`obs`) before fitting.
Extract the $R_t$ posterior as before and plot it alongside the full-data $R_t$ posterior at the same calendar dates.
What happens to the $R_t$ estimates close to the (new) cutoff?
:::

::: {.callout-note collapse="true"}
## Solution

We truncate the observations 14 days before the end of the outbreak and refit the same random walk model:

```{r r_rw_real_time_fit, results = 'hide', message = FALSE}
cutoff <- length(obs) - 14
obs_real_time <- obs[seq_len(cutoff)]

data_real_time <- list(
  n = length(obs_real_time) - 1,
  obs = obs_real_time[-1],
  I0 = inf_ts$infections[1],
  gen_time_max = length(gen_time_pmf),
  gen_time_pmf = gen_time_pmf,
...
r_rw_real_time_fit <- nfidd_sample(
  rw_mod, data = data_real_time, max_treedepth = 12,
  init = \() list(init_R = 1, rw_sd = 0.01)
)
```

We extract the $R_t$ posterior from both fits and plot them together, marking the real-time cutoff:

```{r plot_rt_real_time}
real_time_r_posterior <- r_rw_real_time_fit |>
  gather_draws(R[infection_day]) |>
  ungroup() |>
  mutate(infection_day = infection_day - 1) |>
  filter(.draw %in% sample(.draw, 100)) |>
  mutate(data = "real time")

full_data_r_posterior <- r_rw_inf_fit |>
  gather_draws(R[infection_day]) |>
  ungroup() |>
  mutate(infection_day = infection_day - 1) |>
  filter(.draw %in% sample(.draw, 100)) |>
...
ggplot(
  rbind(full_data_r_posterior, real_time_r_posterior),
  mapping = aes(x = infection_day, y = .value, group = .draw, colour = data)
) +
  geom_line(alpha = 0.1) +
  geom_vline(
    xintercept = max(real_time_r_posterior$infection_day), linetype = "dashed"
  ) +
  scale_colour_brewer(palette = "Set1") +
  labs(
    title = "Real-time vs full-data Rt estimates",
    subtitle = "Random walk model, truncated 14 days before the end of the outbreak"
  ) +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
  theme(legend.position = "bottom")
```

The $R_t$ estimates from the truncated (real-time) fit have much wider uncertainty in the last few days before the cutoff.
The full-data fit, in contrast, sees onsets that happened *after* those days and so estimates $R_t$ at the same calendar dates much more sharply.
The real-time median also tends to flatten out near the cutoff: with no data telling it $R_t$ is changing, the random walk just continues from the last well-informed value, so in real time you can get both wider uncertainty *and* a biased point estimate.
:::
...
-   Review what you have learned in this session with the [learning objectives](../reference/learning_objectives)
