...
One option is to evaluate a single point prediction against a single observation:

$$
\text{error} = \text{prediction} - \text{observation}
$$
You may have seen these kinds of metrics: such as mean absolute error, mean percentage error, or root mean square error.
However, as in the rest of this course, we're focusing on probabilistic forecasts.
This means that what we really need to evaluate is whether we forecast an appropriate range of uncertainty.
...
2. **Unbiasedness**: The forecast should be unbiased. This means that the average forecasted value should be equal to the average observed value. It should not consistently over- or underpredict.
...
::: {.callout-tip}
In this session, we will use "proper" scoring rules: these are scoring rules that make sure no model can get better scores than the *true* model, i.e. the model used to generate the data.
Of course we usually do not know this (as we do not know the "true model" for real-world data) but proper scoring rules incentivise forecasters to make their best attempt at reproducing its behaviour.
...
Before we look in detail at the scores, we can use `summarise_scores` to get a quick overview of the scores (by default calculating a mean across all forecasts).
Do not worry if you do not understand all the scores yet, we will go some of them in more detail in the next section and you can find more information in the [`{scoringutils}` documentation](https://epiforecasts.io/scoringutils/dev).
...
As well as the CRPS we can also look at the calibration and bias of the model [@funkAssessingPerformanceRealtime2019].
