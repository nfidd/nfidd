...
We will now use the mechanistic and statistical models to forecast the number of cases in the future using data simulated in the same way as we did in the [forecast visualisation](forecast-visualisation) session. We will first load in the data and filter for a target forecast date.
...
stat_forecast_fit <- nfidd_sample(
  stat_mod, data = data,
  # again set th  e initial values to make fitting more numerically stable
