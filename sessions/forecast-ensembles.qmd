...
In general, it is not safe to assume that these distributions can be expressed in a simple analytical format as we can do if, e.g. talking about common probability distributions such as the normal or gamma distributions.
...
Additionally, we will use the `hubData` and `hubEnsembles` packages for building ensembles.
...
and define a fourth-root transformation 
```{r fourth-root}
...
Generate the hubverse-style ensemble, glossing over explanation of all the task-id column settings
```{r}
...
hubVis::plot_step_ahead_model_output(
  model_out_tbl = simple_ensembles,
...
::: {.callout-note collapse="true"}
##### Solution
The ensembles are mostly very similar to each other, except for during the peak of the season, when the `fourier` model is predicting something very different from the other forecasts. In those cases, the mean is impacted more, since that single outlying forecast pulls the mean down. However, median remains close to the three other models that have close agreement, at least for the first few horizons.
...
This is a way of representing an observed data point as if it were a prediction from an oracle model. 
The package `hubEvals` expects observations to be stored in this format, so we will first begin by restrucurting the `flu_data` into this format.

```{r make-oracle-output}
target_epiweeks <- unique(all_forecasts$epiweek)
...
Now we are ready to compute some scores
```{r score-ensembles}
...
::: {.callout-note}
The **weighted interval score (WIS)** is a proper scoring rule for quantile forecasts that approximates the **Continuous Ranked Probability Score (CRPS)** by considering a weighted sum of multiple prediction intervals. As the number of intervals increases, the WIS converges to the CRPS, combining sharpness and penalties for over- and under-prediction.[@bracherEvaluatingEpidemicForecasts2021]
