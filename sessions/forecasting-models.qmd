...
## Interactive: a spectrum of models

```{=html}
<iframe class="nfidd-embed" src="../interactive/forecasting-models.html?embed=headline" title="a spectrum of models interactive" loading="lazy" style="width:100%;height:750px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
...
Compare random-walk, statistical and mechanistic forecasts diverging over the horizon. [Open the full explainer ↗](../interactive/forecasting-models.html){target="_blank"}
:::
...
We will now use the mechanistic and statistical models to forecast the number of cases in the future using data simulated in the same way as we did in the [forecasting concepts](forecasting-concepts) session. We will first load in the data and filter for a forecast origin date.
...
In the forecasts above we assumed that we knew the population size roughly. In practice, we may not. Refit the more mechanistic model with different priors for the population size and see how the forecasts change.
