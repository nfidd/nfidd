...
*TBD - need to update `{qra}` package to work with the new version of `{scoringutils}`*

We need to perform quantile regression averaging (QRA) for each forecast date. We also need to consider how many previous forecasts we wish to use to inform each ensemble forecast. Here we decide to use 3 weeks of previous forecasts to inform each QRA ensemble.
