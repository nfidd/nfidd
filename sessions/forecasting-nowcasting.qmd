...
## Step 1: Fit geometric random walk nowcast
...
## Step 2: Extract nowcast point estimates
...
## Step 3: Combine with older "complete" data
...
## Step 4: Forecast using renewal model
...
## Visualise the pipeline forecast
...
The joint model would need to extend the current nowcasting model with:
...
- Use efficient Stan implementations
- Pre-compile models
...
# Combine all forecast results (this would come from implementing the approaches above)
# For now, we'll create placeholder forecast results

# Example structure for forecast results:
# forecast_results <- bind_rows(
#   # Approach 1: Naive
#   naive_forecasts |> mutate(model = "Naive"),
#   # Approach 2: Simple correction  
#   simple_forecasts |> mutate(model = "Simple correction"),
#   # Approach 3: Correction with uncertainty
#   uncertainty_forecasts |> mutate(model = "Simple + uncertainty"),
#   # Approach 4: Joint model
#   joint_forecasts |> mutate(model = "Joint model")
# )
...
For production use, several packages implement these concepts:

- **EpiNow2**: Production-ready joint nowcasting and forecasting
- **epinowcast**: Next-generation framework with flexible model specification  
- **covidcast**: Real-time analysis tools developed during COVID-19

## Key extensions

- **Hierarchical models**: Multiple locations with shared parameters
- **Time-varying delays**: Account for changing reporting patterns
- **External predictors**: Mobility, weather, policy changes
...
1. **Truncation bias is real**: Ignoring recent incomplete data biases forecasts
2. **Model consistency matters**: Using the same generative process for nowcast and forecast avoids mismatch issues [@lison2024]  
3. **Uncertainty propagation**: Joint models properly account for all sources of uncertainty
4. **Trade-offs**: Balance computational cost, accuracy, and interpretability

## Connecting to course themes

This session brings together:
- Proper uncertainty quantification (Sessions 1-2)
- Understanding generative processes (Session 7) 
- Forecast evaluation (Session 9)
- Real-world epidemic analysis challenges
