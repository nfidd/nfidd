...
::: {.callout-note collapse="true"}
## Solution
- The nowcast better matches the ultimately observed data. The geometric random walk allows the model to capture the multiplicative growth in the data and so better capture the generative process of the data.
- This should be particularly true when the data is more truncated (i.e nearer to the date of the nowcast) as the geometric random walk allows the model to extrapolate the growth in the data rather than relying on the prior distribution as the simpler model did.
- However, the model is still quite simple and so may struggle to capture more complex patterns in the data. In particular the prior model for the geometric random walk assumes that onsets are the same as the previous day. This may not be a good assumption in a rapidly changing epidemic (where the reproduction number is not near 1).
:::
...
where $\lambda_{t}$ is the expected number of onsets on day $t$ and $p_{t,d}$ is the probability that an onset on day $t$ is reported on day $t-d$. Here $\lambda_{t}$ is the same as the expected number of onsets on day $t$ in the simple nowcasting model above so we again modelled it using a geometric random walk. We model $p_{t,d}$ as a [Dirichlet distribution](https://distribution-explorer.github.io/multivariate_continuous/dirichlet.html) as it is a distribution over probabilities. $p_{t,d}$ is equivalent to the reporting delays we have been using as fixed quantities so far but now estimated within the model.
...
We also need to update our simulated truth data to inclued the Poisson observation error:
...
::: {.callout-note collapse="true"}
## Solution
- The simple nowcast struggled to capture the generative process of the data and so produced poor nowcasts. The nowcast with the geometric random walk was better but still struggled to capture the generative process of the data. The joint nowcast was the best of the three as it properly handled the uncertainty and allowed us to actually fit the delay distribution.
- However, the joint nowcast is still quite simple and so may struggle to capture more complex patterns in the data. In particular the prior model for the geometric random walk assumes that onsets are the same as the previous day. This may not be a good assumption in a rapidly changing epidemic (where the reproduction number is not near 1).
- In addition, whilst we say it is "quit simple" as should be clear from the code it is actually quite complex and computationally intensive. This is because we are fitting a model to the reporting triangle which is a large data set and so the model is quite slow to fit.
:::
...
As in the [renewal sesssion](R-estimation-and-the-renewal-equation) we need to define the generation time distribution and a incubation period distribution. We will use the same distributions as in the [renewal session](R-estimation-and-the-renewal-equation) for simplicity. These are: 
...
- What we have done here is a very simple version of nowcasting. In practice, nowcasting is a complex process that involves many different steps. For example, we have assumed that the delay distribution is known. In practice, this is often not the case and needs to be estimated from the data. We could do this using methods from the [session on biases in delay distributions](sessions/biases-in-delay-distributions.qmd). Try and do this using the simulated data from the joint nowcast and see how it affects the simple nowcast models.
- The [`epinowcast`](https://package.epinowcast.org/) package implements a more complex version of the model we have used here. It is designed to be highly flexible and so can be used to model a wide range of different data sets. Compare what we did in this session to this [package vignette](https://package.epinowcast.org/articles/single-timeseries-rt-estimation.html).
