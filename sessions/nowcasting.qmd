...
In practice, we often do not know the delay distribution and so need to estimate it from the data. In the [session on biases in delay distributions](sessions/biases-in-delay-distributions.qmd) we saw how we could estimate the delay distribution from the data.
Here we will see what happens if we get the delay distribution wrong.

W use the same data as before but now assume that the delay distribution is a gamma distribution with shape 1 and rate 1.
This is a very different distribution to the lognormal distribution we used to simulate the data.

```{r gamma-dist}
wrong_proportion_reported <- pgamma(1:15, 1, 1)
...
Again we can extract the nowcast onsets and plot them alongside the observed data:
...
::: {.callout-note collapse="true"}
## Solution
- The nowcast now looks very different to the observed data. This is because the model is now using the wrong delay distribution to nowcast the data.
