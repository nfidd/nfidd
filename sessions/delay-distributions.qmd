...
Every chain of infectious disease transmission starts with one person infecting another. But we rarely observe these infection events directly.
...
We often then need to understand how much time has passed between these events. For example, the incubation period might be used to determine the appropriate length of quarantine. 
...
-   Symptoms to hospital admission:
    ...
    -   *Time from symptom onset to hospital admission*: We assume that the symptom onset-to-hospitalisation period is **lognormally** distributed, with **meanlog 1.75** and **sdlog 0.5**, corresponding to a mean delay of about a week.
...
[1] Much like Alice through the Looking Glass [Red Queen](https://en.wikipedia.org/wiki/Red_Queen%27s_race), it can feel like we are running to stay in the same place!
