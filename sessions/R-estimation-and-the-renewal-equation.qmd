...
In the [session on convolutions](using-delay-distributions-to-model-the-data-generating-process-of-an-epidemic#delay-distributions-and-convolutions) we simulated symptom onsets from a time series of infections by convolving with a delay:
...
r_inf_fit <- mod$sample(
  data = data, refresh = 0, show_exceptions = FALSE, show_messages = FALSE, parallel_chains = 4
)
r_inf_fit
```

We can visualise the posteriors in the same way as earlier here and in the [session on convolutions](using-delay-distributions-to-model-the-data-generating-process-of-an-epidemic#estimating-a-time-series-of-infections). For example one could do 
...
Here we have placed a prior on the standard deviation of the random walk, which we have assumed to be half-normal with a mean of 0 and a standard deviation of 0.1. This is a weakly informative prior that allows for some variation in the reproduction number over time but not an unrealistic amount.
...
We can see that the estimates are smoother when using the random walk model for the reproduction number, compared top the normal model. The model that fits directly to infections performs best, which we would expect as it doesn't have to infer the number of infections from symptom onsets but even here the reproduction number estimates are unrealistically noisy due to the assumption of independence between infections each day.
