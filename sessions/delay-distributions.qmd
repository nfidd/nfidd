...
## Interactive: a delay is a distribution

```{=html}
<iframe class="nfidd-embed" src="../interactive/delay-distributions.html?embed=spread" title="delay distributions interactive" loading="lazy" style="width:100%;height:700px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
...
A delay is a distribution, not a single number — see how the spread, not just the mean, shapes what we observe.
:::
...
Posterior ∝ prior × likelihood — watch the posterior form as a compromise between prior and data, and the data take over as they accumulate. This is the inference we run with Stan here.
:::
