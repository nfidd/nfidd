...
This means that our distribution is **double interval censored** which we encountered in the [the biases in delay distribution session](sessions/biases-in-delay-distributions.qmd), so we need to use the same ideas introduced in that session. The important point to remember is that the cumulative distribution function ($F(t)$) of a distribution that has a daily censored primary event can be expressed as,
$$
