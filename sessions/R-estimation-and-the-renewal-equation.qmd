...
This is saved in a snippet called `load-ts.r` which we can run using

```{r load_ts}
source(here("snippets", "load-ts.r"))
head(inf_ts)
```
...
For this we used the `convolved_with_delay()` function which you can load using

```{r add_onsets}
source(here("functions", "convolve-with-delay.r"))
```
...
ggplot(
  ...
  labs(title = "Rt estimates from renewal equation models",
