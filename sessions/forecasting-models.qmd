...
Rather than adding more mechanistic structure to the renewal model, we could add more statistical structure.
...
More specifically, a _stationary_ time series is defined as one that does not have a trend over time.
In infectious disease epidemiology, this would only be expected for endemic diseases without external seasonal influence.
Instead, we would normally consider the reproduction number as a _non-stationary_ time series as it has trends over time.
:::
...
The random walk model is specifically a special case of an AR(1) model where the next value in the time series is predicted as the previous value, multiplied by a value between 1 and -1 , plus some noise. For the log-transformed reproduction number ($ log(R_t) $), the model is:
...
We've coded this up as a stan model in `stan/statistical-r.stan`. See `stan/functions/geometic_diff_ar1.stan` for the function which calculates the reproduction number. Let's load the model:
...
::: {.callout-tip}
## What happens when we are very wrong about the population size? (optional)
...
forecasts |>
  ...
  scale_y_continuous(trans = "log", limits = c(NA, 500)) +
  scale_color_binned(type = "viridis") +
  facet_wrap(~model)
```
...
How do these forecasts compare?
  ...
  - The statistical model does a very poor job of capturing the downturn in the data extending the current trend.
  - The statistical model is more uncertain than the mechanistic model but less uncertain than the random walk.
Which do you prefer?
  - The mechanistic model seems to be the best at capturing the downturn in the data and the uncertainty in the forecasts seems reasonable.
:::
...
- The mechanistic model has the lowest CRPS at all forecast horizon.
- The statistical model performs notably worse at all forecast horizons.
How do the CRPS scores change with forecast date?
- The statistical model does particularly poorly around the peak of the outbreak.
- The mechanistic model does particularly well around the peak of the outbreak versus all other models
:::
...
## Solution
What do you think of the PIT histograms?
- The mechanistic model is reasonably well calibrated but has a tendency to underpredict.
- Both the random walk and statistical model are biased towards overpredicting with the statistical model also being poorly calibrated (liable to both under and overpredict).
- Across horizons the mechanistic model is only liable to underpredict at the longest horizons.
- Similarly, the statistical and random walk models are more biased at longer horizons.
- The forecast date stratified PIT histograms are hard to interpret but the mechanistic model seems to be the best calibrated across forecast date.
:::
...
::: {.callout-note collapse="true"}
## Solution
- The random walk and mechanistic models have much similar performance.
- Even at the peak this is true as the log scale approach more evenly weights the over and underpredictions.
- The statistical model is still notably worse than the other models.
...
## Solution
What do you think of the PIT histograms?
  - The PIT histograms are similar to the original scale PIT histograms.
:::
