...
```{r plot-all-forecasts, eval = FALSE}
forecasts |>
  filter(.draw <= 50) |>
  ggplot(aes(x = day)) +
...
```{r plot-all-forecasts-log, eval = FALSE}
rw_forecasts |>
  filter(.draw <= 50) |>
  ggplot(aes(x = day)) +
  geom_line(aes(y = .value, group = interaction(.draw, target_day), col = target_day), alpha = 0.1) +
...
```{r convert-forecasts, eval = FALSE}
...
```{r pit-histogram-horizon, eval = FALSE}
sc_forecasts |> 
...
```{r pit-histogram-date, eval = FALSE}
 sc_forecasts |>
...
```{r log-convert-forecasts, eval = FALSE}
...
```{r pit-histogram-log, eval = FALSE}
 log_sc_forecasts |>
  get_pit(by = "model") |>
  plot_pit() +
  facet_wrap(~model)
...
```{r pit-histogram-horizon-log, eval = FALSE}
log_sc_forecasts |> 
  mutate(group_horizon = case_when(
    horizon <= 3 ~ "1-3",
    horizon <= 7 ~ "4-7",
...
```{r pit-histogram-date-log, eval = FALSE}
 log_sc_forecasts |>
  get_pit(by = "target_day") |>
  plot_pit() +
  facet_grid(vars(model), vars(group_horizon))
