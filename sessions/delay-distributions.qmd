...
::: {.callout-tip}
Throughout the course, we repeat some small chunks of code. Instead of copying these between sessions, we've put them into functions in the `nfidd` R package that we can use when needed.
:::
...
```{r summarise_naive_delay_model, eval = FALSE}
...
```{r plot_onset_hospitalisation, eval = FALSE}
