...
1. **Complete data at forecast horizon**: What we would observe over the forecast horizon
2. **Filtered data for complete data approach**: Data that is "complete" at the time of forecast
...
- What happens to forecast uncertainty over the forecasthorizon?
...
We start by fitting the a geometric random walk nowcast to the reporting triangle data, as we did in the [joint nowcasting session](sessions/joint-nowcasting).
...
- **Complete data approach**: Performs much worse overall, with CRPS of 182 compared to ~19-20 for the other approaches. Performance degrades rapidly with longer forecast horizons.
- **Pipeline approach**: Shows the best overall performance (CRPS = 18.8) but has some systematic underprediction bias (-0.67).
- **Joint approach**: Slightly worse overall performance (CRPS = 20.5) with some overprediction bias (0.69) and higher dispersion, indicating greater uncertainty.

The pipeline approach performs better here potentially because the reporting delay is relatively short and the central nowcast estimate was accurate.
In this case, having less uncertainty was beneficial.
However, this is not guaranteed - the higher dispersion in the joint approach reflects more realistic uncertainty quantification.
If the nowcast had been less accurate or reporting delays longer, the joint approach might have performed better.
...
Research has demonstrated that joint modelling of data sources provides significant advantages over combining estimates from separate models [@lison2024] when evaluating multiple nowcast dates.
