...
A key insight is that, at the current day $T$, the number of reports $n_{t, d}$ for each reference day $t$ at delay $d$ splits into those we have **already observed** (short delays, $d \leq T-t$) and those still **missing** (long delays, $d > T-t$). Summing over delay, the eventual total $N_t$ therefore splits into an observed and a missing part:
...
## Interactive: the reporting triangle filling in

```{=html}
<iframe class="nfidd-embed" src="../interactive/nowcasting.html?embed=triangle" title="the reporting triangle filling in interactive" loading="lazy" style="width:100%;height:1250px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
...
The reporting triangle and fill-in from the nowcasting session — here the delay itself must also be estimated from the same data.
:::
...
- When reporting delays don't provide enough information to identify recent trends, using the underlying generative model (renewal process) prevents bias
