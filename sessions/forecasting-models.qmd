...
In this session we will use the `fpp3` package to fit and forecast ARIMA models, the `dplyr` and `tidyr` packages for data wrangling, and `ggplot2` library for plotting.
...
We will now use the function `gg_tsdisplay()` to give a unique view into our dataset.
```{r}
gg_tsdisplay(flu_data, y = wili, plot_type='partial', lag_max = 104)
```
...
There is a lot of additional examples and explanation about interpreting ACF and PCF plots in [FPP3, Chapter 9.5](https://otexts.com/fpp3/non-seasonal-arima.html). 
...
This is one of the places where the stationarity assumption is important.

Here is an equation for an ARIMA(1,1,0) model:
$$ y_t - y_{t-1} = \mu_t  + \phi_1 (y_{t-1} - y_{t-2}) + \varepsilon_t.$$
...
The above two forms of ARIMA models are fairly easy to conceptualize, but the moving average part is a little bit trickier. Basically, a moving average model uses residuals from the previous timepoints as predictors in the model. 
...
Here is an example of an ARIMA(0,0,1) model:
...
# Fit a targeted ARIMA model

Ok, enough math! Let's get back to our data and fit a model, now that we have introduced the components of an ARIMA model more clearly. 
...
```{r}
gg_tsdisplay(flu_data, y = difference(wili), plot_type='partial', lag_max = 104)
```
...
Based on this plot, let's put on our list of candidate models to fit an ARIMA(2,1,0). 
...
Through a lot of experimentation, we've found that a fourth-root transformation often does a reasonable job at stabilizing variance.
...
Let's fit four models total: ARIMA(2,0,0) and ARIMA(2,1,0), each with and without the fourth_root transformation.
```{r}
fits <- flu_data |>
  model(
    arima200 = ARIMA(wili ~ pdq(2,0,0)),
    arima210 = ARIMA(wili ~ pdq(2,1,0)),
    arima200_trans = ARIMA(my_fourth_root(wili) ~ pdq(2,0,0)),
    arima210_trans = ARIMA(my_fourth_root(wili) ~ pdq(2,1,0))
  )
forecast(fits, h=40) |>
...
2. Include Fourier series terms (sums of sine and cosine terms with a fixed periodicity of 365 days) to fit annual periodic trends.
...
Note that you can just add a `fourier()` term to your model equation and this will add the Fourier Series terms to the model predictors. 
...
Well, that `arima200_fourier` model can likely be improved, but we have finally arrived at a forecast model that at least passes a visual sanity check.
