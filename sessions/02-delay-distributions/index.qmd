...
The aim of this session is for you to familiarise yourself with the concept of probability distributions and how to estimate their parameters using `stan`.
...
In R, this is done using a family of functions that start with `r`.
For example, to simulate from a normal distribution you would use the `rnorm()` function.
All these functions have a first argument `n`, the number of random replicates to generate.

```{r rnorm}
rnorm(n = 10, mean = 0, sd = 1)
```

We will use probability distribution to characterise epidemiological delays.
...
These are quite similar, with one difference being that the lognormal commonly has a "heavier tail" on the right, i.e. gives more probability to occasional very large values, whereas the gamma distribution as more of a tail on the left, i.e. gives more probability to values lower than the mean.
...
A lognormal distribution with mean 5 and variance 2, for example, has (after a bit of calculation) $\mu = 1.57$ and $\sd = 0.28$.
...
We can then use this in conjunction with the gamma distributed random numbers generated earlier to see if we can recover the parameters of the gamma distribution used.
...
Once you have familiarised yourself with the model, you can use it to conduct inference
