...
reporting_delay <- censored_delay_pmf(rlnorm, max = 15, meanlog = 1, sdlog = 0.5)
```
...
reporting_triangle <- onset_df |>
  filter(day < 40) |>
  mutate(
    reporting_delay = list(tibble(d = 0:15, reporting_delay = reporting_delay)
  )) |>
...
As we only partially observe the reporting triangle we need to filer it to only include the data we have observed:
...
In the previous sessions we have seen how to estimate the reproduction number and how to nowcast the data. We can now put these two pieces together to estimate the reproduction number and nowcast the data jointly. We will also estimate the delay distribution from the data.
