...
We need to perform quantile regression averaging (QRA) for each forecast date. We also need to consider how many previous forecasts we wish to use to inform each ensemble forecast. Here we decide to use up to 3 weeks of previous forecasts to inform each QRA ensemble.
...
We might also want to create a QRA ensemble separately for each forecast horizon. We can do this using `qra()` with the `group` argument.
